...
# ARM code
CC := gcc
CFLAGS := -O3 -mfpu=vfpv3 -mfloat-abi=hard -march=armv7 -I./include -I. 
LDFLAGS := /usr/lib/arm-linux-gnueabihf/libgfortran.so.3 -l:blas_LINUX.a -l:cblas_LINUX.a -l:libblas.so.3 -l:liblapacke.a -l:liblapack.so.3
# LDFLAGS := -lprussdrv

SRCS := main.c prussdrv.c adcdriver_host.c spidriver_host.c svd.c matrixmul.c subf.c xnorm.c construction.c
OBJS := main.o prussdrv.o adcdriver_host.o spidriver_host.o svd.o matrixmul.o subf.o xnorm.o construction.o
EXES := main
INCLUDEDIR := ./include
...
main.o: $(SRCS) /usr/include/fftw3.h svd.h matrixmul.h subf.h construction.h
...
svd.o: svd.c /usr/include/lapacke.h /usr/include/cblas.h 
	echo "--> Building svd.o"
	$(CC) $(CFLAGS) -c $< -o $@
...
subf.o: subf.c svd.h matrixmul.h xnorm.h
	echo "--> Building subf.o"
	$(CC) $(CFLAGS) -c $< -o $@

xnorm.o: xnorm.c
	echo "--> Building xnorm.o"
	$(CC) $(CFLAGS) -c $< -o $@

construction.o: construction.c
	echo "--> Building construction.o"
	$(CC) $(CFLAGS) -c $< -o $@

# Link the ARM objects
main: $(OBJS) 
	echo "--> Linking ARM stuff...."
	$(CC) $(CFLAGS) $^ $(LIBLOCS) $(LDFLAGS) -lm -l:/libfftw3.a -o $@ 
